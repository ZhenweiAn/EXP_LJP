...
class Label_Infer():
    def __init__(self, config):
        ...
        print('start loading')
        self.model.load_state_dict(torch.load('../models/2021-08-19-08:22:34.bin'))
...
def pre_process(config):
    ...
    print(exp_sent_num)
    with open('../data/exp_train.json','w',encoding='utf-8') as f:
        json.dump([Tensor_List[:train_index],Label_List[:train_index], Crim_List[:train_index]],f,indent=4,ensure_ascii=False)
    with open('../data/exp_test.json','w',encoding='utf-8') as f:
        json.dump([Tensor_List[train_index:],Label_List[train_index:], Crim_List[train_index:]],f,indent=4,ensure_ascii=False)

def exp_train(config):
    with open('../data/exp_train.json','r',encoding='utf-8') as f:
        train_data = json.load(f)
    with open('../data/exp_test.json','r',encoding='utf-8') as f:
    ...
    for epoch in range(config.epochs):
        ...
        mi_f1  = exp_evaluate_multi_label(Model, data_loader['train'],config)
...
def F1_each_focus(golden_labels, predict_labels,config):
...
def metrics(golden_labels, predict_labels, name='罪名'):
    ...
    print("mi_f1: ",mi_f1)
    print("ma_f1: ",ma_f1)
    F1_each_focus(golden_labels,predict_labels,config)
